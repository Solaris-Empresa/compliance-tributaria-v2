...
function leiLabel(lei: string) {
  const map: Record<string, string> = {
    ...
    lc224: "LC 224/2024",
    ec132: "EC 132/2023",
    lc123: "LC 123/2006 — Simples Nacional",
    lc116: "LC 116/2003",
    lc87: "LC 87/1996",
    cg_ibs: "CG-IBS",
    rfb_cbs: "RFB-CBS",
    ...
    resolucao_cgsn_140: "Resolução CGSN 140/2018",
    moc_cte_v4: "Manual CT-e v4 (SEFAZ)",
    moc_mdfe_v3: "Manual MDF-e v3 (SEFAZ)",
  };
...
const RFCS = [
  {
    id: "RFC-001",
    title: "Fusão chunks 810+811 (lc227)",
    lei: "lc227",
    severity: "P2",
    status: "EXECUTED",
    ids: "810–811",
  ...
  {
    id: "RFC-002",
    title: "25 chunks migrados lc214→lc123",
    lei: "lc123",
    severity: "P1",
    status: "EXECUTED",
    ids: "664–722 (25 chunks)",
  ...
  {
    id: "RFC-003",
    title: "Artigos de leis avulsas na faixa 617–779",
    lei: "lc214",
    severity: "P3",
    status: "BACKLOG",
  ...
  {
    id: "MIG-001",
    title: "source depreciado como ENUM — permanece varchar(20)",
    lei: "lc214",
    severity: "P3",
    status: "EXECUTED",
    ids: "project_gaps_v3.source",
    action:
      "✅ Executada — source é varchar(20), não ENUM. Novo valor 'engine' cabe sem migration.",
    sprint: "T",
    created: "2026-04-05",
    approved: true,
  },
...
const SPRINTS = [
  ...
  },
  {
    ...
    commit: "dbad765",
    changes: [
      "G8 companyProfile no briefing",
      "G7 RAG 4 queries paralelas",
      "Fix CI jobs",
    ],
    status: "done",
  },
  ...
  },
  {
    ...
    changes: [
      ...
      "6/6 casos validados — Dr. José Rodrigues (M1 baseline)",
      "Governança v4.1: CODEOWNERS 15 entradas + 12 workflows CI",
      "1.476 testes passando · 0 erros TypeScript · 16 PRs mergeados",
      "Evidence Pack M1 — gate triplo aprovado (Técnico + Jurídico + P.O.)",
    ],
    status: "done",
  },
  {
    id: "Sprint V — PV-01 Lote 1",
    date: "2026-04-05",
    pr: "#328",
    commit: "49c3f68",
    changes: [
      "Lote 1: +10 casos NCM/NBS → 16 confirmados (NCM:9 · NBS:7)",
      "Fix nbs-engine.ts extractFonte (suporte a artigo string direta)",
      "Testes decision-kernel: 26/26 passando",
      "ESTADO-ATUAL v4.4 (PR #329)",
    ],
    status: "done",
  },
  {
    id: "Sprint V — PV-01 Lote 2",
    date: "2026-04-05",
    pr: "#330",
    commit: "1c42774",
    changes: [
      "Lote 2: +8 casos NCM/NBS → 24 confirmados (NCM:12 · NBS:12)",
      "Correção S-07: planos saúde Arts.234-235 (não Arts.193-199)",
...
const SOURCE_FILES = [
  {
    path: "server/rag-retriever.ts",
    role: "Motor de recuperação RAG (LIKE + topicos + cnaeGroups)",
    critical: true,
  },
  {
    path: "server/rag-corpus.ts",
    role: "Corpus estático de 63 artigos",
    critical: true,
  },
  {
    path: "server/routers/ragInventory.ts",
    role: "tRPC endpoint ragInventory.getSnapshot — cockpit ao vivo",
    critical: true,
  },
  {
    path: "scripts/corpus-utils.mjs",
    role: "Utilitários de ingestão — buildAnchorId, upsertChunk",
    critical: true,
  },
  {
    path: "scripts/migrate-anchor-id-legado.mjs",
    role: "Migração de anchor_id para chunks legados",
    critical: false,
  },
  {
    path: "drizzle/schema.ts",
    role: "Schema ragDocuments — enum lei, anchor_id UNIQUE",
    critical: true,
  },
  {
    path: "docs/rag/CORPUS-BASELINE.md",
    role: "Fonte de verdade do estado do corpus (documento vivo)",
    critical: true,
  },
  {
    path: "docs/rag/RAG-GOVERNANCE.md",
    role: "Regras de governança — RFCs, rollback, métricas",
    critical: false,
  },
  {
    path: "docs/rag/RFC/CORPUS-RFC-001.md",
    role: "RFC G-01: id 811 fragmentado",
    critical: false,
  },
  {
    path: "docs/rag/RFC/CORPUS-RFC-002.md",
    role: "RFC G-02: ids 617–779 campo lei",
    critical: false,
  },
  {
    path: "docs/rag/gold-set-queries.sql",
    role: "8 queries canônicas de validação de cobertura",
    critical: false,
  },
  // Sprint N
  {
    path: "server/lib/solaris-gap-analyzer.ts",
    role: "G17: módulo isolado de análise SOLARIS — INSERT com transação",
    critical: true,
  },
  {
    path: "server/config/feature-flags.ts",
    role: "Feature flags — g15-fonte-perguntas, g17-solaris-analyzer, bloqueios",
    critical: false,
  },
  {
    path: "docs/adr/0001-g17-modulo-isolado.md",
    role: "ADR-0001: G17 módulo isolado (Sprint N)",
    critical: false,
  },
  {
    path: "docs/adr/0002-arquitetura-3-ondas-perguntas.md",
    role: "ADR-0002: Arquitetura 3 Ondas de Perguntas (Sprint N)",
    critical: false,
  },
  {
    path: "docs/governance/post-mortems/2026-03-31-g17-insert-silencioso.md",
    role: "Post-mortem G17: INSERT silencioso — 5 Whys + DORA Sprint N",
    critical: false,
  },
  {
    path: ".github/workflows/validate-implementation.yml",
    role: "CI: Gates v5.0 — Q6/Q7/R9/R2 automáticos",
    critical: false,
  },
  // Sprint T — Milestone 1 (Decision Kernel)
  {
    path: "server/lib/decision-kernel/datasets/ncm-dataset.json",
    role: "Dataset NCM — 19 casos confirmados · Sprint V Lote 1+2+3 (PRs #328, #330, #333)",
    critical: true,
  },
  {
    path: "server/lib/decision-kernel/datasets/nbs-dataset.json",
    role: "Dataset NBS — 19 casos confirmados · Sprint V Lote 1+2+3 (PRs #328, #330, #333)",
    critical: true,
  },
  {
    path: "server/lib/decision-kernel/engine/decision-kernel.test.ts",
    role: "48 testes Q5 decision-kernel · Sprint V Lote 1+2+3",
    critical: false,
  },
  {
    path: "server/lib/decision-kernel/engine/ncm-engine.ts",
    role: "Engine determinístico NCM — lookup regime tributário LC 214/2025",
    critical: true,
  },
  {
    path: "server/lib/decision-kernel/engine/nbs-engine.ts",
    role: "Engine determinístico NBS — lookup regime tributário LC 214/2025",
    critical: true,
  },
  {
    path: "server/lib/engine-gap-analyzer.ts",
    role: "Integrador engine → project_gaps_v3 (fire-and-forget, source='engine')",
    critical: true,
  },
];
...
function ScoreRing({
  ...
  const color =
    value >= target ? "#22c55e" : value >= 85 ? "#f59e0b" : "#ef4444";
  return (
    ...
    <div>
        ...
        {[
          {
            ...
            sub: `${totalLeis} leis ativas`,
            color: "#22c55e",
          },
          {
            label: "Leis ativas",
            value: String(totalLeis),
            sub: snapshot?.by_lei?.map((r: any) => r.lei).join(" · ") ?? "—",
            color: "#6366f1",
          },
          {
            label: "Anomalias críticas",
            value: String(anomalies),
            sub: anomalies === 0 ? "corpus íntegro" : "requer RFC",
            color: anomalies > 0 ? "#f59e0b" : "#22c55e",
          },
          {
            label: "Sem anchor_id",
            value: String(semAnchor),
            sub: semAnchor === 0 ? "100% rastreável" : "orphans detectados",
            color: semAnchor > 0 ? "#f59e0b" : "#22c55e",
          },
          {
            label: "Gold set verde",
            value: `${goldOk}/${goldTotal}`,
            sub: "queries canônicas",
            color: goldOk === goldTotal ? "#22c55e" : "#f59e0b",
          },
          {
            label: "Confiabilidade",
            value: `${corpusConfidence}%`,
            sub: "meta: 98%",
            color: corpusConfidence >= 98 ? "#22c55e" : "#f59e0b",
          },
        ].map(k => (
          <div
            key={k.label}
      ...
      >
        ...
        <div>
          ...
          {[
            {
              label: "Integridade do corpus",
              value: `${totalChunks.toLocaleString("pt-BR")} chunks · ${semAnchor} orphans · ${semAnchor === 0 ? "100%" : "parcial"} anchor_id`,
              ok: semAnchor === 0,
            },
            {
              label: "Distribuição de leis",
              value: `${totalLeis} leis ativas`,
              ok: totalLeis >= 4,
            },
            {
              label: "Gold set canônico",
              value: `${goldOk}/${goldTotal} verde · ${corpusConfidence}% confiabilidade`,
              ok: goldOk === goldTotal,
            },
            {
              label: "Anomalias críticas",
              value:
                anomalies === 0
                  ...
                  : `${anomalies} anomalia(s) — requer RFC`,
              ok: anomalies === 0,
            },
          ].map(s => (
            <div
      ...
      >
        ...
        Nenhum UPDATE no banco sem (1) dry-run documentado, (2) aprovação do
        Orquestrador, (3) aprovação do P.O. e (4) SQL de rollback na RFC.
      </div>
    </div>
  );
}
            ...
            <div key={c.lei} style={{ marginBottom: 14 }}>
              ...
              >
                <div
                  style={{
                    width: `${w}%`,
                    background: c.status === "ok" ? "#6366f1" : "#f59e0b",
                    height: "100%",
                    borderRadius: 4,
              ...
              >
                <span style={{ color: "#475569", fontSize: 11 }}>
                  {w.toFixed(1)}% do corpus · ids {c.idMin.toLocaleString()}–
                  {c.idMax.toLocaleString()}
                </span>
                <span
                  style={{
          ...
          <tbody>
            {CORPUS.map((c: any, i: number) => (
              <tr
                ...
                style={{
                  borderBottom: "1px solid #0f172a",
                  background: i % 2 === 0 ? "#0b1423" : "#0f172a",
                }}
              >
                <td
                  style={{
                ...
                <td style={{ padding: "10px 14px" }}>
                  ...
                  >
                    {c.chunks > 0
                      ? `${((c.anchor / c.chunks) * 100).toFixed(0)}%`
                      : "—"}
                  </span>
                </td>
                <td
                ...
                <td style={{ padding: "10px 14px" }}>
                  <Badge
                    text={c.status === "ok" ? "✓ Íntegro" : "⚠ Anomalia"}
                    color={c.status === "ok" ? "#22c55e" : "#f59e0b"}
                    bg={c.status === "ok" ? "#052e16" : "#431407"}
                  />
                </td>
              </tr>
            ))}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {GOLD_SET.map((g: any) => (
          <div
            ...
            style={{
              background: "#0f172a",
              border: `1px solid ${g.status === "ok" ? "#14532d" : "#451a03"}`,
              borderRadius: 10,
              padding: "12px 16px",
              display: "flex",
              alignItems: "center",
      ...
      >
        ...
        Queries executadas em tempo real contra o banco. GS-07b é informativo
        (chunks SUPERSEDED de governança) e não entra no cálculo de confidence.
      </div>
    </div>
  );
}
...
function AnomaliesTab({ anomaliesData }: { anomaliesData: any[] }) {
  const rfcOpen = RFCS.filter(
    r => r.status === "DRAFT" || r.status === "OPEN" || r.status === "BACKLOG"
  );
  return (
    <div>
      <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
        {[
          {
            label: "P0 Crítico",
            count: anomaliesData.filter((a: any) => a.bytes < 5).length,
            color: "#ef4444",
          },
          {
            label: "P1 Alto",
            count: rfcOpen.filter(r => r.severity === "P1").length,
            color: "#f97316",
          },
          {
            label: "P2 Médio",
            count: rfcOpen.filter(r => r.severity === "P2").length,
            color: "#eab308",
          },
          {
            label: "P3 Baixo",
            count: rfcOpen.filter(r => r.severity === "P3").length,
            color: "#6b7280",
          },
        ].map(p => (
          <div
            ...
            style={{
              background: "#0f172a",
              border: `1px solid ${p.count > 0 ? p.color + "44" : "#1e293b"}`,
              borderRadius: 10,
              padding: "12px 18px",
              textAlign: "center",
              flex: 1,
          ...
          >
            <div
              style={{
                color: p.count > 0 ? p.color : "#334155",
                fontSize: 28,
                fontWeight: 700,
                fontFamily: "monospace",
              }}
            ...
            <div
              style={{
                color: p.count > 0 ? p.color + "bb" : "#334155",
                fontSize: 11,
              }}
            >
              {p.label}
          ...
          >
            <thead>
              <tr style={{ borderBottom: "1px solid #1e293b" }}>
                {["ID", "Lei", "Artigo", "Bytes", "anchor_id", "Autor"].map(
                  h => (
                    <th
                      key={h}
                      style={{
              ...
              {anomaliesData.map((a: any) => (
                <tr key={a.id} style={{ borderBottom: "1px solid #0f172a" }}>
                  ...
                  <td
                    style={{
                      padding: "6px 10px",
                      color: a.anchor_id ? "#22c55e" : "#ef4444",
                      fontFamily: "monospace",
                    }}
                  >
                    {a.anchor_id ? "✓" : "NULL"}
      ...
      {RFCS.map(r => (
        <div
          ...
          style={{
            background: "#0f172a",
            border: `1px solid ${sev[r.severity]}44`,
            borderRadius: 12,
            padding: 18,
            marginBottom: 12,
          }}
        >
          ...
          >
            <div>
              <span style={{ color: "#475569" }}>Lei: </span>
              <span style={{ color: "#a5b4fc", fontFamily: "monospace" }}>
                {r.lei}
              </span>
            </div>
            <div>
              <span style={{ color: "#475569" }}>IDs afetados: </span>
              <span style={{ color: "#94a3b8", fontFamily: "monospace" }}>
                {r.ids}
              </span>
            </div>
            <div style={{ gridColumn: "span 2" }}>
              <span style={{ color: "#475569" }}>Ação: </span>
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
            <Badge
              text={
                r.approved ? "✓ P.O. aprovado" : "⏳ Aguarda aprovação P.O."
              }
              color={r.approved ? "#22c55e" : "#f59e0b"}
              bg={r.approved ? "#052e16" : "#431407"}
            />
            <Badge
              text="Dry-run obrigatório antes do UPDATE"
              color="#6366f1"
              bg="#1e1b4b"
            />
          </div>
    ...
    <div>
      ...
      {byAutor.length === 0 ? (
        <div style={{ color: "#475569", fontSize: 13, padding: "20px 0" }}>
          Nenhum dado de rastreabilidade disponível.
        </div>
      ) : (
        <div
          style={{
        ...
        >
          ...
          >
            <thead>
              ...
              >
                {["Autor", "Chunks", "id_min", "id_max", "Última revisão"].map(
                  h => (
                    <th
                      key={h}
                      style={{
    ...
    <div>
      ...
      {SPRINTS.map(s => (
        ...
        >
          ...
          >
            ...
            />
            <div
              style={{ flex: 1, width: 1, background: "#1e293b", marginTop: 4 }}
            />
          </div>
          <div style={{ flex: 1 }}>
            <div
      ...
      >
        ...
        {[
          ...
          >
            <span style={{ color: "#334155", fontSize: 13, marginTop: 1 }}>
              ☐
            </span>
...
function FilesTab() {
  return (
    <div>
      <div style={{ color: "#64748b", fontSize: 12, marginBottom: 16 }}>
        Arquivos fonte do sistema RAG · arquivos críticos têm impacto direto na
        recuperabilidade do corpus
      </div>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                ...
                {f.critical && (
                  <Badge text="crítico" color="#6366f1" bg="#1e1b4b" />
                )}
              </div>
      ...
      >
        ...
        <span style={{ color: "#475569" }}>
          qualquer alteração em arquivos marcados como crítico exige revisão do
          Orquestrador, testes de regressão e atualização do CORPUS-BASELINE.md.
        </span>
      </div>
    </div>
  );
    ...
    <div>
      ...
      >
        ...
        <div style={{ color: "#64748b", fontSize: 12, marginBottom: 14 }}>
          O campo <code style={{ color: "#a5b4fc" }}>autor</code> em todos os
          chunks permite rollback cirúrgico por sprint ou por RFC.
        </div>
        {[
          {
            title: "Rollback de ingestão completa",
            code: `DELETE FROM ragDocuments\nWHERE autor = 'ingestao-sprint-g-2026-03-26';`,
          },
          {
            title: "Rollback de RFC-002 (se executada)",
            code: `UPDATE ragDocuments\nSET lei = 'lc214',\n    autor = 'ingestao-automatica-sprint-d'\nWHERE id BETWEEN 617 AND 779\n  AND autor = 'correcao-rfc-002-sprint-g';`,
          },
          {
            title: "Verificação pós-rollback",
            code: `SELECT lei, COUNT(*) FROM ragDocuments\nWHERE id BETWEEN 617 AND 779\nGROUP BY lei;\n-- Deve mostrar somente lc214 com 163 registros`,
          },
        ].map(r => (
      ...
      >
        ...
        >
          Fallback de recuperação (sem rollback de banco)
        </div>
        <div style={{ color: "#64748b", fontSize: 12, marginBottom: 8 }}>
          Se uma lei retornar 0 chunks, o{" "}
          <code style={{ color: "#a5b4fc" }}>rag-retriever.ts</code> pode
          ampliar a busca de topicos para conteudo:
        </div>
        <pre
          style={{
            background: "#0a0f1a",
...
export default function RAGCockpit() {
  ...
  const {
    ...
    error,
    refetch,
  } = trpc.ragInventory.getSnapshot.useQuery(
    undefined,
  ...
  const goldOk = snapshot
    ? snapshot.gold_set.filter((g: any) => g.status === "ok").length
    : 0;
  ...
  const CORPUS =
    snapshot?.by_lei.map((r: any) => ({
      ...
      note:
        Number(r.sem_anchor) > 0 ? `${r.sem_anchor} sem anchor_id` : "Íntegro",
    })) ?? [];
  ...
  const panels = [
    <OverviewTab
      ...
      goldOk={goldOk}
      goldTotal={goldTotal}
      corpusConfidence={corpusConfidence}
      anomalies={anomalies}
      snapshot={snapshot}
    />,
    <CorpusTab
      ...
      >
        ...
        <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
          ...
          <div style={{ textAlign: "center" }}>
            ...
            >
              {totalChunks.toLocaleString("pt-BR")}
            </div>
      ...
      {anomalies > 0 && (
        ...
        >
          <span style={{ color: "#f97316", fontWeight: 700 }}>
            ⚠ {anomalies} anomalia{anomalies > 1 ? "s" : ""} crítica
            {anomalies > 1 ? "s" : ""} detectada{anomalies > 1 ? "s" : ""}:{" "}
          </span>
          <span style={{ color: "#fca5a5" }}>
            chunks com conteúdo &lt; 10 bytes ou sem anchor_id — requer RFC.
          </span>
        </div>
      )}
