...
import { autoConfirmPerfil } from "../lib/archetype/autoConfirmPerfil";
import {
  validateM1Seed,
...
export const perfilRouter = router({
  ...
  confirm: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Mud.1 (#1562) — persist extraído ao helper compartilhado autoConfirmPerfil
      // (mesmo ponto de escrita usado pelo auto-confirm no confirmCnaes). Pré-condições
      // (409 / confirmável / 0 HARD_BLOCK) já validadas acima.
      return autoConfirmPerfil({
        db,
