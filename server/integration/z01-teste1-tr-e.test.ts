...
type DiagnosticResult = {
  gaps: GapLike[];
  riscos: RiscoLike[];
  briefing?: any; // Adicionado para compatibilidade com o retorno do runFullDiagnostic
};

type ProjetoInput = {
  ncmAnswers?: Array<{ ncm: string; fonte: string; fonte_ref: string; lei_ref: string; resposta: 'sim' | 'nao' }>;
  solarisAnswers?: Array<{ id: string; lei_ref: string; resposta: 'sim' | 'nao' }>;
  nbsAnswers?: Array<{ nbs: string; fonte: string; fonte_ref: string; lei_ref: string; resposta: 'sim' | 'nao' }>;
  iagenAnswers?: Array<{ id: string; fonte_ref: string; lei_ref: string; gap: string }>;
...
// Helper local generateBriefingFromResult
function generateBriefingFromResult(result: DiagnosticResult) {
  const content: any = {
    ...
    cnaes: [], // CNAEs não estão no DiagnosticResult, será um array vazio ou mockado
    lei_refs: [...new Set(result.gaps.map(g => g.lei_ref).filter(Boolean))],
  };

  const fontes_usadas = [
    ...new Set(result.gaps.map(g => ({ tipo: g.fonte, fonte_ref: g.fonte_ref })))
  ];
...
// Dados dos projetos P-T1 a P-T5
const projetos = {
  'P-T1': {
...
describe('TESTE1-TR-E: TR-21 a TR-25 (Rastreabilidade end-to-end 3 niveis)', () => {
  it('TR-21: Deve identificar gaps e riscos para o Projeto P-T1 (produto puro - alimentos)', async () => {
    const result = await runFullDiagnostic(projetos['P-T1']);
    // P-T1: 3 gaps
    expect(result.gaps).toHaveLength(3);
    expect(result.riscos).toHaveLength(3);
    expect(result.riscos[0].severity).toBe('medio'); // Art. 14 LC 214/2025
    expect(result.riscos[1].severity).toBe('alto');  // Art. 2 LC 214/2025

    const briefing = generateBriefingFromResult(result);
    expect(briefing.total_riscos).toBe(3); // P-T1: 3 gaps
    expect(briefing.gaps).toHaveLength(3);
    expect(briefing.riscos).toHaveLength(3);
  ...
  it('TR-22: Deve identificar gaps e riscos para o Projeto P-T2 (servico puro - consultoria TI)', async () => {
    const result = await runFullDiagnostic(projetos['P-T2']);
    expect(result.gaps).toHaveLength(3);
    expect(result.riscos).toHaveLength(3);
    expect(result.riscos[0].severity).toBe('medio'); // Art. 19 LC 214/2025
    expect(result.riscos[1].severity).toBe('medio'); // Art. 19 LC 214/2025
    expect(result.riscos[2].severity).toBe('medio'); // Art. 11 LC 214/2025
  ...
  it('TR-23: Deve identificar gaps e riscos para o Projeto P-T3 (mista farmacia)', async () => {
    const result = await runFullDiagnostic(projetos['P-T3']);
    // P-T3: 5 gaps
    // P-T5: 6 gaps
    expect(result.gaps).toHaveLength(5);
    expect(result.riscos).toHaveLength(5);
    expect(result.riscos[0].severity).toBe('medio'); // Art. 34 LC 214/2025
    expect(result.riscos[1].severity).toBe('medio'); // Art. 34 LC 214/2025
    expect(result.riscos[2].severity).toBe('medio'); // Art. 29 LC 214/2025
    expect(result.riscos[3].severity).toBe('medio'); // Art. 34 LC 214/2025

    const briefing = generateBriefingFromResult(result);
    expect(briefing.total_riscos).toBe(5);
    expect(briefing.gaps).toHaveLength(5);
    expect(briefing.riscos).toHaveLength(5);
  ...
  it('TR-24: Deve identificar gaps e riscos para o Projeto P-T4 (produto IS - bebidas)', async () => {
    const result = await runFullDiagnostic(projetos['P-T4']);
    // P-T3: 5 gaps
    // P-T5: 6 gaps
    expect(result.gaps).toHaveLength(4);
    expect(result.riscos).toHaveLength(4);
    expect(result.riscos[0].severity).toBe('alto');  // Art. 2 LC 214/2025
    expect(result.riscos[1].severity).toBe('alto');  // Art. 2 LC 214/2025
    expect(result.riscos[2].severity).toBe('alto');  // Art. 2 LC 214/2025
    expect(result.riscos[3].severity).toBe('alto');  // Art. 2 LC 214/2025

    const briefing = generateBriefingFromResult(result);
    expect(briefing.total_riscos).toBe(4);
    expect(briefing.gaps).toHaveLength(4);
    expect(briefing.riscos).toHaveLength(4);
  ...
  it('TR-25: Deve identificar gaps e riscos para o Projeto P-T5 (mista tech - hardware+software)', async () => {
    const result = await runFullDiagnostic(projetos['P-T5']);
    // P-T5: 6 gaps
    expect(result.gaps).toHaveLength(6);
    expect(result.riscos).toHaveLength(6);
    expect(result.riscos[0].severity).toBe('medio'); // Art. 9 LC 214/2025
    expect(result.riscos[1].severity).toBe('medio'); // Art. 9 LC 214/2025
    expect(result.riscos[2].severity).toBe('medio'); // Art. 11 LC 214/2025
    expect(result.riscos[3].severity).toBe('medio'); // Art. 11 LC 214/2025
    expect(result.riscos[4].severity).toBe('medio'); // Art. 9 LC 214/2025

    const briefing = generateBriefingFromResult(result);
    expect(briefing.total_riscos).toBe(6); // P-T5: 6 gaps
    expect(briefing.gaps).toHaveLength(6);
    expect(briefing.riscos).toHaveLength(6);
