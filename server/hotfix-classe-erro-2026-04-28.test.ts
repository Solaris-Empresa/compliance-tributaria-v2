...
describe("Hotfix Classe de Erro — Dataset Expansion 2026-04-28 (Opção A, 4 NCMs)", () => {
  ...
  it("NCM 2202.10.00 (refrigerante) regression — continua bebida com IS=true (chapter 22 NÃO vira combustível)", () => {
    const result = deriveObjetoFromNcm("2202.10.00");
    expect(result.objeto).toBe("bebida"); // CRÍTICO: NÃO pode virar "combustivel"
    expect(result.blocker).toBeNull();
  });

  // === Eligibility gates (R6 indireto) ===

  it("isCategoryAllowed('imposto_seletivo','industria') === ALLOWED para combustível", () => {
    const elig = isCategoryAllowed("imposto_seletivo", "industria");
    expect(elig.allowed).toBe(true);
  });

  it("isCategoryAllowed('imposto_seletivo','agronegocio') === BLOCKED para milho", () => {
