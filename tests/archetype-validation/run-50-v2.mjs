...
const suite = JSON.parse(
  fs.readFileSync(path.join(__dirname, "M1-arquetipo-50-casos-brasil-v2.json"), "utf8")
);
...
function determineStatus(arquetipo, blockers, missingFields, seed) {
  ...
  // AMBIGUOUS: hard blockers disparados (V-01 a V-04) — seed nao fornece campos exigidos
  // Multi-natureza SEM setor regulado = composicao normal (nao AMBIGUOUS)
  // Multi-natureza COM setor regulado ja eh coberto por V-04 nos blockers
  if (blockers.length > 0) return "AMBIGUOUS";
