...
**HEAD:** `f8a5864` · **Data:** 2026-04-09 · **Versão:** v2.01 · **Fonte da verdade:** [GitHub main](https://github.com/Solaris-Empresa/compliance-tributaria-v2/blob/f8a58640c04a6774fe3672e8cc8b48f92482820f/)
...
>
> **v2.01 — Correções aplicadas (Orquestrador 2026-04-09):**
> - CRÍTICO-01: Gap Engine adicionado como camada intermediária entre Consolidadores e Pipeline LLM
> - CRÍTICO-02: Score CPIE marcado com ⚠️ AUDIT-C-004 (decisão P.O. pendente)
> - CRÍTICO-03: Matrizes de Risco marcadas [AS-IS] com nota de substituição Z-07
> - MENOR-01: Link `#Lgrep` inválido removido da linha do RAG

---

## Diagrama do Fluxo E2E (v2.01)
...
> - **Score CPIE** é output explícito do Consolidador de Diagnóstico ⚠️ AUDIT-C-004
> - **Gap Engine** adicionado como camada intermediária (CRÍTICO-01 — v2.01)
...
flowchart TD
    ...
    %% ── GAP ENGINE (CRÍTICO-01 — adicionado em v2.01) ──────────────────────
...
| 📤 Gaps Unificados | `server/routers-fluxo-v3.ts` | L1820 | [→ GitHub](https://github.com/Solaris-Empresa/compliance-tributaria-v2/blob/f8a58640c04a6774fe3672e8cc8b48f92482820f/server/routers-fluxo-v3.ts#L1820) | Lista de gaps tributários identificados por camada (QC, QO, CNAE). Base para o Gap Engine. |
| 🔧 Gap Engine | `server/lib/` | — | [→ GitHub](https://github.com/Solaris-Empresa/compliance-tributaria-v2/blob/f8a58640c04a6774fe3672e8cc8b48f92482820f/server/lib/) | Camada intermediária entre Consolidadores e Pipeline LLM. Composta por `solaris-gap-analyzer.ts`, `iagen-gap-analyzer.ts` e `risk-categorizer.ts`. Produz `GapConfirmed[]` — input real do `risk-engine-v4.ts` (Z-07). |
| 📤 Riscos | `server/routers-fluxo-v3.ts` | L1113 | [→ GitHub](https://github.com/Solaris-Empresa/compliance-tributaria-v2/blob/f8a58640c04a6774fe3672e8cc8b48f92482820f/server/routers-fluxo-v3.ts#L1113) | 4 matrizes de risco (Contabilidade, Negócio, TI, Jurídico) geradas por LLM com RAG específico por área. Armazenadas em project_risks_v3. **[AS-IS — Z-07 substitui por 10 categorias LC 214/2025]** |
| 📤 Score CPIE | `server/routers-fluxo-v3.ts` | L1820 | [→ GitHub](https://github.com/Solaris-Empresa/compliance-tributaria-v2/blob/f8a58640c04a6774fe3672e8cc8b48f92482820f/server/routers-fluxo-v3.ts#L1820) | Score de compliance 0–100 calculado a partir dos gaps e completude do diagnóstico. ⚠️ AUDIT-C-004 aberto — cálculo aguarda decisão P.O. (Opção A vs B) |
...
| 🏗️ RAG (2.509 chunks) | `server/rag.ts` | — | [→ GitHub](https://github.com/Solaris-Empresa/compliance-tributaria-v2/blob/f8a58640c04a6774fe3672e8cc8b48f92482820f/server/rag.ts) | Retrieval-Augmented Generation: 2.509 chunks de 10 leis da Reforma Tributária. Usado em generateBriefingFromDiagnostic e generateRiskMatrices. |
...
| Componente | Arquivo | Linha | GitHub | Objetivo |
|-----------|---------|-------|--------|----------|
| 🏗️ RAG Engine | `server/rag.ts` | — | [→ GitHub](https://github.com/Solaris-Empresa/compliance-tributaria-v2/blob/f8a58640c04a6774fe3672e8cc8b48f92482820f/server/rag.ts) | 2.509 chunks · 10 leis da Reforma Tributária |
| 🏗️ CNAE Embeddings | `server/cnae-embeddings.ts` | — | [→ GitHub](https://github.com/Solaris-Empresa/compliance-tributaria-v2/blob/f8a58640c04a6774fe3672e8cc8b48f92482820f/server/cnae-embeddings.ts) | Embedding semântico · 8 operationTypes (FIX_02) |
| 🏗️ Risk Engine | `server/riskEngine.ts` | — | [→ GitHub](https://github.com/Solaris-Empresa/compliance-tributaria-v2/blob/f8a58640c04a6774fe3672e8cc8b48f92482820f/server/riskEngine.ts) | Cálculo matemático probability × impact |
| 🏗️ Schema DB | `drizzle/schema.ts` | — | [→ GitHub](https://github.com/Solaris-Empresa/compliance-tributaria-v2/blob/f8a58640c04a6774fe3672e8cc8b48f92482820f/drizzle/schema.ts) | TiDB Cloud · tabelas principais do sistema |
...
| `generateRiskMatrices` | L1113 | mutation | **Matrizes de Risco** — 4 áreas · RAG por área **[AS-IS · alvo hot-swap Z-07]** |
...
|--------|------|-------|----------|
| v1.00 | 2026-04-09 | Manus | Versão inicial — Gate B |
| v2.00 | 2026-04-09 | Manus | Renomeação (sem correções aplicadas) |
| v2.01 | 2026-04-09 | Manus | **CRÍTICO-01:** Gap Engine adicionado ao diagrama e tabela · **CRÍTICO-02:** Score CPIE marcado ⚠️ AUDIT-C-004 · **CRÍTICO-03:** Riscos marcados [AS-IS] com nota Z-07 · **MENOR-01:** Link `#Lgrep` removido do RAG |
