/**
 * scripts/build-corpus-moc-cte-anexo1-v4.ts
 *
 * Chunker determinístico do MOC CT-e Anexo I v4.00 (Leiaute e Regras de Validação).
 * Corpus Onda 2 — SCAFFOLD (infraestrutura, sem conteúdo).
 *
 * Input:  scripts/corpus-source/moc_cte_anexo1_v4.txt  (aguarda extração pelo Manus)
 * Output: server/rag-corpus-moc-cte-anexo1-v4.ts
 * Lei:    moc_cte_anexo1_v4 (enum em drizzle/schema.ts — migration 0095)
 *
 ...
 *
 * Uso: pnpm exec tsx scripts/build-corpus-moc-cte-anexo1-v4.ts
 */
...
const LEI_SLUG = "moc_cte_anexo1_v4";
const SOURCE_FILE = resolve(
  root,
  "scripts/corpus-source/moc_cte_anexo1_v4.txt"
);
const OUTPUT_FILE = resolve(root, "server/rag-corpus-moc-cte-anexo1-v4.ts");
...
function main(): void {
  ...
  const result = buildCorpus({
    ...
    lei: LEI_SLUG,
    exportName: "RAG_CORPUS_MOC_CTE_ANEXO1_V4",
    headerComment:
      "Corpus RAG — MOC CT-e Anexo I v4.00 (Leiaute e Regras de Validação)\n" +
